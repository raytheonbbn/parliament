...
On Windows, we use Microsoft's \path|vcpkg| package manager to download and create the RocksDB library and its dependencies. In your working copy, chnage to the \path|rocksdb-windows| directory. If you are running Windows on an Intel- or AMD-based machine, issue these commands from a Visual Studio terminal:
...
export ROCKSDB_HOME=/opt/rocksdb-10.10.1
export ZSTD_HOME=/opt/zstd-1.5.7
\end{verbatim}}

Now download and expand the source code archive for RocksDB version \rocksdbversion{} from the releases page of the RocksDB GitHub site\urlcite{https://github.com/facebook/rocksdb/releases}. Change line 1395 of util/compression.cc from this:
...
{\small\begin{verbatim}
make static_lib shared_lib PREFIX=/opt/rocksdb-10.10.1 \
   ...
   -L${ZSTD_HOME}/lib"
sudo make install PREFIX=/opt/rocksdb-10.10.1 SKIP_DEPENDS=1
\end{verbatim}}

Finally, run this test on the resulting dynamic library to be sure the build worked correctly:

{\small\begin{verbatim}
otool -L $ROCKSDB_HOME/lib/librocksdb.10.10.1.dylib
\end{verbatim}}

This should result in output like the following:

{\small\begin{verbatim}
.../librocksdb.10.10.1.dylib (architecture x86_64):
...
.../librocksdb.10.10.1.dylib (architecture arm64):
   librocksdb.10.10.dylib ...
   @rpath/libgflags.2.3.dylib ...
   /usr/lib/libz.1.dylib ...
   /usr/lib/libbz2.1.0.dylib ...
...
{\small\begin{verbatim}
ldd $ROCKSDB_HOME/lib/librocksdb.so.10.10.1
\end{verbatim}}
