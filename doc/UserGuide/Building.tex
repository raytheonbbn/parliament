...
\ac{pmnt} uses \ac{lmdb}\urlcite{http://www.lmdb.tech/doc/}, an embedded database manager from the OpenLDAP Project\urlcite{https://www.openldap.org/project/}.  \ac{lmdb} is licensed under a BSD-style license called the OpenLDAP Public License\urlcite{https://www.openldap.org/software/release/license.html}.  The following procedures are based on \ac{lmdb} version \lmdbversion.

Navigate to the releases page of \acp{lmdb} GitHub site\urlcite{https://github.com/LMDB/lmdb/releases}. Click the Tags button and download version \lmdbversion. Switch to the \path|lmdb| directory of your \ac{pmnt} working copy and expand the archive there. Issue these commands:
...
Once you have built and installed Berkeley DB, define the following environment variable so that the \ac{pmnt} build infrastructure can find the libraries:
