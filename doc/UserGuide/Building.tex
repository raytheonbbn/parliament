...
\ac{pmnt} uses RocksDB\urlcite{https://rocksdb.org/}, an embedded persistent key-value store from Meta. RocksDB is dual-licensed under both the GPLv2 and Apache 2.0 Licenses. In the following procedures, wherever ``«ver»'' appears, you should substitute the current RocksDB version, which is \rocksdbversion{} as of this writing.
...
export ROCKSDB_HOME=/opt/rocksdb-«ver»
...
{\small\begin{verbatim}
make static_lib shared_lib PREFIX=/opt/rocksdb-«ver» \
   ...
   -L${ZSTD_HOME}/lib"
sudo make install PREFIX=/opt/rocksdb-«ver» SKIP_DEPENDS=1
\end{verbatim}}

Finally, run this test on the resulting dynamic library to be sure the build worked correctly:

{\small\begin{verbatim}
otool -L $ROCKSDB_HOME/lib/librocksdb.«ver».dylib
\end{verbatim}}

This should result in output like the following:

{\small\begin{verbatim}
.../librocksdb.«ver».dylib (architecture x86_64):
   librocksdb.«ver».dylib ...
...
.../librocksdb.«ver».dylib (architecture arm64):
   librocksdb.«ver».dylib ...
   @rpath/libgflags.2.3.dylib ...
   /usr/lib/libz.1.dylib ...
   /usr/lib/libbz2.1.0.dylib ...
...
{\small\begin{verbatim}
ldd $ROCKSDB_HOME/lib/librocksdb.so.«ver»
\end{verbatim}}
...
--with-container --with-date_time --with-filesystem --with-log
--with-regex --with-serialization --with-test --with-thread
define=BOOST_LOG_USE_STD_REGEX define=BOOST_LOG_WITHOUT_SYSLOG
define=BOOST_LOG_WITHOUT_IPC define=BOOST_LOG_WITHOUT_ASIO
define=BOOST_TEST_ALTERNATIVE_INIT_API toolset=gcc address-model=64
variant=debug,release link=shared,static runtime-link=shared
architecture=x86 cxxstd=20 stage
\end{verbatim}}
...
\begin{itemize}
	\item\verb|BOOST_ARCHITECTURE|: \emph{(Linux only)} Either \texttt{x64} or \texttt{a64}.
	...
	\item\verb|BOOST_BUILD_PATH|: The directory \path|$BOOST_ROOT/tools/build|.
	...
	\item\verb|UBUNTU_USR_LIB|: \emph{(Linux only)} The directory under which third party libraries are typically installed. Typically either \path|/usr/lib/x86_64-linux-gnu| or \path|/usr/lib/aarch64-linux-gnu|.
