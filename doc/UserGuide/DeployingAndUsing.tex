...
Pre-built binaries for common platforms are available on the \ac{pmnt} Web site\urlcite{https://github.com/\githubOrg/parliament/releases}.  You can also build \ac{pmnt} yourself (see Chapter~\ref{chapter-building-parliament}).
...
\emph{Memory configuration:} \ac{pmnt} uses three separate pools of memory:
...
\begin{itemize}
	\item Don't make the Java heap too big. Many Java programmers are accustomed to setting their Java heap as large as possible, but for \ac{pmnt}, the Java heap is not as important as the memory-mapped files.

	\item The Java heap size should be significantly less than the total memory available, in order to leave sufficient memory for the memory-mapped files.
\end{itemize}
...
\begin{description}
	...
	\item[uriToIntFileName] The name of the file containing the mapping from numeric resource identifiers to resource strings.  This file is managed by RocksDB. \emph{Default: \path|u2i.db|}
	...
	\item[bdbCacheSize] This obsolete setting is now ignored.
...
set CATALINA_OPTS=-Djava.library.path=%PMNT_BIN%
set CATALINA_OPTS=%CATALINA_OPTS% -Xmx%JAVA_HEAP_SIZE%
\end{verbatim}}
...
\begin{enumerate}
	\item You will need \path|KbGraph.jar| and \path|Parliament.jar| from your binary distribution, along with the Jena jar files, which you can download from the Jena web site\urlcite{https://jena.apache.org/}.  Place all of these jar files in a convenient location and ensure that they are added to your class path.
	...
	\item To ensure that the \ac{jvm} can find the \acp{dll} or shared libraries, add their directory to the Java system property \verb|java.library.path|.
...
\begin{itemize}
	...
	\begin{itemize}
		\item The Boost and RocksDB \acp{dll}:  In the deployment procedures detailed in this document, these \acp{dll} should reside in the same location as \ac{pmnt} itself.
