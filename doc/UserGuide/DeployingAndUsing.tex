...
The file \path|ParliamentKB/conf/jetty.xml| configures the Jetty servlet container.  Generally, there is no need to modify this file.  Other \ac{pmnt} configuration files are discussed in Section~\ref{section-parliament-config-file}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The \ac{pmnt} Configuration Files}
\label{section-parliament-config-file}

\ac{pmnt} has two primary configuration files.  The first configures \acp{pmnt} native code logging and is called \path|ParliamentLogConfig.txt|.  This one is less important to everyday \ac{pmnt} users, and will be discussed second.  The more important file (\path|ParliamentKbConfig.txt|) contains settings pertaining to storage, query, and inference.  (Note that in both cases, these are the typical names for these files, but they can be named anything.)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\acp{pmnt} Main Configuration File}
\label{section-parliament-main-config-file}

Here is how \ac{pmnt} finds \path|ParliamentKbConfig.txt|:

\begin{enumerate}
	\item If the environment variable \verb|PARLIAMENT_KB_CONFIG_PATH| is set, its value is assumed to be the configuration file's absolute path.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\acp{pmnt} Logging Configuration File}
\label{section-parliament-log-config-file}

\acp{pmnt} second configuration file, \path|ParliamentLogConfig.txt|, has a format similar to \path|ParliamentKbConfig.txt|.  \ac{pmnt} also finds it in a similar way, with environment variable \verb|PARLIAMENT_LOG_CONFIG_PATH| and file name \path|ParliamentLogConfig.txt|.  The recognized settings are:
\begin{description}
