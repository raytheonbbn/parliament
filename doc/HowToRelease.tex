...
\begin{verbatim}
   cd jena/docker
   ant -D../../target/distro/parliament-2.8.0-ubuntu20-64.zip
   cp ../../target/distro/
         parliament-2.8.0-ubuntu-docker.tar.bz2 release
\end{verbatim}
...
\testStep Verify that there are 3 instances in the geospatial index.

\testStep Explore a bit to ensure that functionality works as expected.

\testStep Run the following queries:
...
\begin{verbatim}
   env LD_LIBRARY_PATH=../bin
      PARLIAMENT_LOG_CONFIG_PATH=../ParliamentLogConfig.txt
      PARLIAMENT_KB_CONFIG_PATH=../ParliamentKbConfig.txt
...
On Mac: Substitute \texttt{DYLD\_LIBRARY\_PATH} for \texttt{LD\_LIBRARY\_PATH} in the commands above.
...
\testStep Install as service (Windows and Linux) according to the instructions in the User Guide.

\testStep Start as service (Windows and Linux) or Daemon (Mac)

\testStep Run one of the queries above.

\testStep Shut down the service or daemon.

\testStep Uninstall the service (Windows and Linux).

\testStep Start the Docker container.

\testStep Insert some data.

\testStep Run a couple queries.

\testStep Stop the container.
...
\begin{verbatim}
   ...
   git tag -a -m "Dependencies archive released on
         2022-04-25 prior to releasing version 2.8.0"
