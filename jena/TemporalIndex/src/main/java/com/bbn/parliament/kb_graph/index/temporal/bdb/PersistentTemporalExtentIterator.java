...
ClosableIterator<Record<TemporalExtent>> {
	...
	/**Used in filtering iterable results. */
	private final Class<? extends TemporalExtent> typefilter;
	...
	public PersistentTemporalExtentIterator(PersistentTemporalIndex index,
			...
			long minEnd, long maxEnd, Class<? extends TemporalExtent> typefilter) {
	...
	private boolean testCurrent() {
		...
		if (!typefilter.isInstance(current.getValue()))	{
			return false;
		}
