...
public class ResultSetIterator implements ClosableIterator<Record<Geometry>> {
   ...
   public ResultSetIterator(Connection c, ResultSet rs, String nodeColumn,
                            String geometryColumn) {
      this.c = c;
      this.resultSet = rs;
      this.nodeCol = nodeColumn;
      this.geometryCol = geometryColumn;
      this.pos = 0;
   }
   ...
   public boolean hasNext() {
      try {
         ...
         throw new RuntimeException(
                                    "Error while checking whether result set has next",
                                    e);
         // hasNext = false;
         // return false;
      }
   ...
   public Node getNode() {
      try {
         return GeometryConverter.getNodeRepresentation(resultSet
               .getString(nodeCol));
      } catch (SQLException e) {
         LOG.error("Error while moving to next row", e);
         close();
