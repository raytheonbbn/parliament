...
if ($foreground) {
	$argList = @(
		...
		"-Dlog4j.configuration=file:$pmntDirFwdSlash/config/log4j.foreground.properties",
	...
	if ($install) {
		...
		$argList = @(
			...
			'++JvmOptions', """-Dlog4j.configuration=file:$pmntDirFwdSlash/config/log4j.daemon.properties""",
