...
public class StressTestSuite
{
	...
	public static void performTests() throws Exception
	{
			...
			for (int p = 0; p < _writerPercents.length; p++)
			{
				...
				for (int o = 0; o < _operationCounts.length; o++)
				{
					...
					for (int t = 0; t < _threadCounts.length; ++t)
					{
						...
						if (_bLocal)
						{
							(st = new LocalStressTest(_threadCounts[t], opCount,
								percentage, _csv)).doit();
						}
						else
						{
							(st = new RemoteStressTest(_threadCounts[t], opCount,
								percentage, _csv)).doit();
						}
						timeInMilliseconds += (st.getTotalElapsedTime() / 1000000);
						// store results for printing after all thread sets are
						// complete
						tpArray[t + 1][o + 1] = format.format(st.getThroughput());
						rtArray[t + 1][o + 1] = format.format(st
